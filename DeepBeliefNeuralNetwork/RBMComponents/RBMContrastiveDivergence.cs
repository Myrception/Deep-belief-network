...
namespace DeepBeliefNeuralNetwork.RBMComponents
{
    internal class RBMContrastiveDivergence
    {
        ...
        private int a = 1;
        public int CalculateHiddenBinaryState(double bias, RBMNeurons actualHiddeNeuron, RBMWeightMatrix weightMatrix, List<RBMNeurons> visualLayer, int weightMatrixRow , Random _rnd)
        {
            double sum = 0;
        ...
        public int CalculateHiddenBinaryStateWithProbability(double bias, RBMNeurons actualHiddeNeuron, RBMWeightMatrix weightMatrix, List<RBMNeurons> visualLayer, int weightMatrixRow, Random _rnd)
        {
            double sum = 0;
            for (int i = 0; i < weightMatrixRow; i++)
            {
        ...
        public int CalculateReconstructionVisualBinaryStateDrivenByData(double bias, RBMNeurons actualVisualNeuron, RBMWeightMatrix weightMatrix, List<RBMNeurons> hiddenLayer, int weightMatrixRow,int weightMatrixColoumn, Random _rnd)
        {
        ...
        public int CalculateReconstructionVisualBinaryStateDrivenByReconstruction(double bias, RBMNeurons actualVisualNeuron, RBMWeightMatrix weightMatrix, List<RBMNeurons> hiddenLayer, int weightMatrixRow, int weightMatrixColoumn, Random _rnd)
        {
            double sum = 0;
            for (int i = weightMatrixRow; i < weightMatrixRow + weightMatrixColoumn; i++)
            {
